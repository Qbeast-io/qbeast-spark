...
package object qbeast {
  ...
  private[qbeast] val defaultCubeSize: ConfigEntry[Int] =
    ConfigBuilder("spark.qbeast.index.defaultCubeSize")
      .version("0.2.0")
      .intConf
      .createWithDefault(5000000)

  private[qbeast] val cubeWeightsBufferCapacity: ConfigEntry[Long] =
    ConfigBuilder("spark.qbeast.index.cubeWeightsBufferCapacity")
      .version("0.2.0")
      .longConf
      .createWithDefault(100000L)

  private[qbeast] val defaultNumberOfRetries: ConfigEntry[Int] =
    ConfigBuilder("spark.qbeast.index.numberOfRetries")
      .version("0.2.0")
      .intConf
      .createWithDefault(2)

  private[qbeast] val stagingSizeInBytes: OptionalConfigEntry[Long] =
    ConfigBuilder("spark.qbeast.index.stagingSizeInBytes")
      .version("0.2.0")
      .longConf
      .createOptional

  private[qbeast] val columnsToIndexSelectorEnabled: ConfigEntry[Boolean] =
    ConfigBuilder("spark.qbeast.index.columnsToIndex.auto")
      .version("0.2.0")
      .booleanConf
      .createWithDefault(false)

  private[qbeast] val maxNumColumnsToIndex: ConfigEntry[Int] =
