...
import io.qbeast.spark.sources.catalog.{QbeastCatalogUtils, TableCreationMode}
import io.qbeast.spark.sources.v2.{QbeastStagedTableImpl, QbeastTableImpl}
...
class QbeastCatalogTest extends QbeastIntegrationTestSpec with CatalogTestSuite {
  ...
  it should "replace table" in withQbeastContextSparkAndTmpWarehouse((spark, _) => {
    ...
    val newSchema = schema.add(StructField("newCol", IntegerType, nullable = false))
  ...
  it should "alter table" in withQbeastContextSparkAndTmpWarehouse((spark, _) => {
    ...
    val modifiedSchema = StructType(schema.fields ++ Seq(StructField("x", IntegerType, nullable = false)))
