...
class SparkRevisionFactoryTest extends QbeastIntegrationTestSpec {
  ...
  it should "createNewRevision with only one columns" in withSpark(spark => {
    ...
    val qid = QTableID("t")
    val options =
      QbeastOptions(Map(QbeastOptions.COLUMNS_TO_INDEX -> "a", QbeastOptions.CUBE_SIZE -> "10"))
    val revision = SparkRevisionFactory.createNewRevision(qid, schema, options)
  ...
  it should "createNewRevision with or without transformation types" in withSpark(spark => {
    import spark.implicits._
    val schema = spark.range(1).map(i => T3(i, i * 2.0, s"$i", i * 1.2f)).schema
    val qid = QTableID("t")
    val options = QbeastOptions(
      Map(QbeastOptions.COLUMNS_TO_INDEX -> "a,b,c,d", QbeastOptions.CUBE_SIZE -> "10"))
    val revisionWithoutTType = SparkRevisionFactory.createNewRevision(qid, schema, options)

    revisionWithoutTType.tableID shouldBe qid
    revisionWithoutTType.revisionID shouldBe 0
    revisionWithoutTType.desiredCubeSize shouldBe 10
    revisionWithoutTType.columnTransformers shouldBe Vector(
    ...
    revisionWithoutTType.transformations shouldBe Vector()

    val revisionWithTType =
      SparkRevisionFactory.createNewRevision(
        qid,
        schema,
        options.copy(columnsToIndex = Seq("a:linear", "b:linear", "c:hashing", "d:linear")))

    revisionWithTType.copy(timestamp = 0) shouldBe revisionWithoutTType.copy(timestamp = 0)
  })
//
...
//  it should "createNewRevision with columnStats even on APPEND mode" in withSparkAndTmpDir(
//    (spark, tmpDir) => {
//      import spark.implicits._
//      val df = spark.range(10).map(i => T3(i, i * 2.0, s"$i", i * 1.2f))
//      val columnStats = """{ "a_min": 0, "a_max": 20 }"""
...
//    })
  ...
  it should "createNewRevision using all hashing" in withSpark(spark => {
    import spark.implicits._
    val schema = spark.range(1).map(i => T3(i, i * 2.0, s"$i", i * 1.2f)).schema
    val qid = QTableID("t")
    val options = QbeastOptions(
