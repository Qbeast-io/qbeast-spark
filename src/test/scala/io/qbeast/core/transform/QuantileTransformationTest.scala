...
class QuantileTransformationTest extends AnyFlatSpec with Matchers {

  val orderedDataTypeTest = IntegerDataType

  behavior of "QuantileTransformation"

  "transform" should "return correct transformation for found index" in {
    val qt = QuantileTransformation(IndexedSeq(1, 2, 3), orderedDataTypeTest)
  ...
  "isSupersededBy" should "return true when new transformation is not default and current is default" in {
    val qt = QuantileTransformation(orderedDataTypeTest.defaultQuantiles, orderedDataTypeTest)
    val newQt = QuantileTransformation(IndexedSeq(1, 2, 3), orderedDataTypeTest)
  ...
  it should "return false when new transformation is default and current is not default" in {
    val qt = QuantileTransformation(IndexedSeq(1, 2, 3), orderedDataTypeTest)
    val newQt = QuantileTransformation(orderedDataTypeTest.defaultQuantiles, orderedDataTypeTest)
    qt.isSupersededBy(newQt) should be(false)
  }

  it should "return true when quantiles are different and neither is default" in {
    val qt = QuantileTransformation(IndexedSeq(1, 2, 3), orderedDataTypeTest)
    val newQt = QuantileTransformation(IndexedSeq(4, 5, 6), orderedDataTypeTest)
    qt.isSupersededBy(newQt) should be(true)
  }
  ...
  "merge" should "return other transformation when it is a QuantileTransformation" in {
    val qt = QuantileTransformation(IndexedSeq(1, 2, 3), orderedDataTypeTest)
    val otherQt = QuantileTransformation(IndexedSeq(4, 5, 6), orderedDataTypeTest)
