...
class IndexFilesTest extends QbeastIntegrationTestSpec {
  ...
  "IndexFiles" should "be able to create an AddFile instance from IndexFile" in withSpark { _ =>
    ...
    val addFile = QbeastFiles.toAddFile(dataChange = true)(indexFile)
  ...
  it should "be able to create an IndexFile instance from an AddFile" in withSpark { _ =>
    ...
    val indexFile = QbeastFiles.fromAddFile(2)(addFile)
  ...
  it should "transform the DeleteFile to a Remove File" in withSpark { _ =>
    val indexFile = DeleteFile(path = "path", size = 2L, deletionTime = 0L)

    val dataChange = false
    val removeFile = QbeastFiles.toRemoveFile(dataChange = dataChange)(indexFile)
  ...
  it should "be able to create a FileStatus from an IndexFile" in withSpark { _ =>
    ...
    val indexStatus = QbeastFiles.toFileStatus(indexPath)(indexFile)
  ...
  it should "be able to create a FileStatusWithMetadata from IndexFile" in withSpark { _ =>
    ...
    val indexPath = new Path("/absolute/")
    val indexStatus =
      QbeastFiles.toFileStatusWithMetadata(indexPath, Map("key" -> "value"))(indexFile)
  ...
  it should "throw error when trying to create an IndexFile with a wrong block format start" in withSpark {
    _ =>
      ...
      an[JsonParseException] shouldBe thrownBy(QbeastFiles.fromAddFile(2)(addFile))
  }
  ...
  it should "throw error when trying to create an IndexFile with a wrong block format end" in withSpark {
    _ =>
      ...
      an[NumberFormatException] shouldBe thrownBy(QbeastFiles.fromAddFile(2)(addFile))
  }
