 ...
 */
package io.qbeast.spark.snapshot
 ...
 */
trait SparkQbeastSnapshot extends QbeastSnapshot {
...
object SparkQbeastSnapshot {
   ...
   */
  def apply(tableFormat: String, tableID: QTableID): SparkQbeastSnapshot = {
    tableFormat match {
      case "delta" =>
        new DeltaQbeastSnapshot(tableID)
      // TODO: Take into account more table formats
      case _ =>
        throw new IllegalArgumentException(s"Unsupported table format: $tableFormat")
    }
  }
