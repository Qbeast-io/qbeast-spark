...
private[spark] class StagingDataManager(tableID: QTableID) extends DeltaStagingUtils {
  ...
  /**
   * <<<<<<< HEAD Stage the data without indexing by writing it in the delta format. If the table
   * is not yet a qbeast table, use ConvertToQbeastCommand for conversion after the write.
   * \======= Stage the data without indexing by writing it in the delta format. If the table is
   * not yet a qbeast table, use ConvertToQbeastCommand for conversion after the write. >>>>>>>
   * main-1.0.0
   *
   * @param data
   *   the data to stage
