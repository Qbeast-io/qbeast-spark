 ...
 */
package io.qbeast.spark.delta.index.query
...
class QueryExecutorTest extends QbeastDeltaTestSpec with QueryTestSpec {
  ...
  it should "handle index with a missing inner cube" in withSparkAndTmpDir((spark, tmpdir) => {
    ...
    val outputBlocks = queryExecutor invokePrivate executeRevision(querySpec, faultyIndexStatus)
    val matchCubes = outputBlocks.toList.toDS().select(struct("*").as("cubeId"))
