...
import io.qbeast.spark.delta.{DeltaPreCommitHook, DeltaQbeastHookLoader}
...
class QbeastHookLoaderTest extends AnyFlatSpec with Matchers {
  ...
  "loadHook" should "create a simple hook when only the hook class name is provided" in {
    ...
    val hookOpts = qbeastOptions.hookInfo.map(DeltaQbeastHookLoader.loadHook)
  ...
  it should
    "create a stateful hook when both hook class name and arg are provided" in {
      ...
      val hooks = qbeastOptions.hookInfo.map(DeltaQbeastHookLoader.loadHook)
