...
class TestContentLoader(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.checkpoint_file_path = os.path.join(
            log_path, "00000000000000000001.checkpoint.parquet"
        )
    ...
    def check_empty_revision(self, transformers: list[str]) -> None:
        ...
        self.assertEqual(
            set(transformers).pop(), "io.qbeast.core.transform.EmptyTransformer"
        )

    def test_metadata_extraction_from_json_files(self):
        metadata = extract_metadata_from_json_files(
            [self.first_log_file_path], "qbeast.revision.0"
        )
        transformers = list(
            map(lambda t: t["className"], metadata["columnTransformers"])
        )
        self.check_empty_revision(transformers)

    def test_metadata_extraction_from_checkpoint_file(self):
        metadata = extract_metadata_from_checkpoint(
            self.checkpoint_file_path, "qbeast.revision.0"
        )
        transformers = list(
            map(lambda t: t["className"], metadata["columnTransformers"])
        )
    ...
    def test_addFile_extraction_from_json_log_files(self):
        (add_files, remove_paths) = addFiles_from_json_log_files(
            self.json_log_files_with_remove, "1"
        )
        self.assertTrue(len(add_files) > len(remove_paths))

        valid_add_files = list(
            filter(lambda add_file: add_file["path"] not in remove_paths, add_files)
        )
